...
namespace MyWorkTracker.Code
{
    public class MWTController
    {
        ...
        public void ImportData(Dictionary<PreferenceName, string> loadPreferences, List<WorkItemStatus> importStatuses, XDocument xml)
        {
            ...
            var query3 = from element in xml.Descendants("WorkItem")
                         select element;
            ...
            foreach (var el3 in query3)
            {
                ...
                if (el3.Element("DeletionDate").Value != null)
        ...
        public void ExportToXML(string dbConn, string exportTo, bool exportSettings, string exportWorkItemOptionString, int daysStale, bool includeDeleted, bool onlyLastStatus, bool onlyLastDueDate)
        {
        ...
        private protected XElement GetWorkItemStatusTableAsXML(string dbConnString, bool onlyLastStatus, string workItemIDs = "-1")
        {
            ...
            XElement workItemStatusChangeXML = null;
            using (var connection = new SQLiteConnection(dbConnString))
            {
                using (var cmd = new SQLiteCommand(connection))
                {
                    ...
                    if (onlyLastStatus == false)
                        cmd.CommandText += ", CreationDateTime DESC";
                    ...
                    using (SQLiteDataReader reader = cmd.ExecuteReader())
                    {
                        while (reader.Read())
                        {
                            ...
                            workItemStatusChangeXML = new XElement(new XElement("StatusChange", new XAttribute("WorkItemStatus_ID", Convert.ToInt32(reader["WorkItemStatus_ID"]))));
        ...
        private protected XElement GetWorkItemTableAsXML(out string wiIDString, string dbConnString, bool activeOnly = false, int daysStale = 9999, bool includeDeleted = false)
        {
                ...
                using (var cmd = new SQLiteCommand(connection))
                {
                    ...
                    using (SQLiteDataReader reader = cmd.ExecuteReader())
                    {
                        while (reader.Read())
                        {
                            ...
                            workItemXML = new XElement("WorkItem", new XAttribute("WorkItem_ID", workItemID),
            ...
            Console.WriteLine("----------------------------------------------------");
            Console.WriteLine(allWorkItemsXML);
            Console.WriteLine("----------------------------------------------------");
